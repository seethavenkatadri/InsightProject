...
def fetch(limit):
   ...
   # sql = "select flight, ST_AsGeoJSON(ST_GeomFromText('POINT(latitude longitude)', 4326)) from (select flight_id AS flight,info ->> 'latitude' as latitude, info ->> 'longitude' as longitude  from flight limit %s) a";
    sql = "select flight, $${""type"": ""Feature"", ""geometry"": { ""type"": ""Point"",""coordinates"": [$$ || longitude ||','|| latitude || $$]},""properties"": {""name"": ""$$ || flight || $$"" }}$$ from (select flight_id AS flight,info ->> 'latitude' as latitude, info ->> 'longitude' as longitude  from flight order by create_date desc limit %s) a;"
    print(sql)
    conn = None
    state_id = None
    try:
